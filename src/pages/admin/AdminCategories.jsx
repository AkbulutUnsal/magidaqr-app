...
/* ───────────────────────────────────────────────────────────
   magidaQR · Kategoriler  (qrmenum referans · yeşil tema #1D9E75)
   Şema-uyumlu: image_url / is_active / group / outlet_id kolonları
   mevcut satırlardan otomatik algılanır; yoksa gizlenir.
   App.jsx: /admin/categories rotasını <AdminCategories /> yap.
─────────────────────────────────────────────────────────── */
...
export default function AdminCategories() {
  ...
  async function load() {
    ...
    const [{ data: c }, { data: items }, { data: out }] = await Promise.all([
  ...
  const groupField = GROUP_FIELDS.find(k => k in sample) || null
  const hasOutlet = 'outlet_id' in sample
  const groupOptions = useMemo(() => {
    if (!groupField) return []
    return [...new Set(cats.map(c => c[groupField]).filter(Boolean))]
  }, [cats, groupField])

  const visible = useMemo(() => {
    if (!hasOutlet || !outletFilter) return cats
    return cats.filter(c => c.outlet_id === outletFilter)
  }, [cats, outletFilter, hasOutlet])
  ...
  async function remove(cat) {
    ...
    const msg = n > 0
      ? `"${dispName(cat)}" kategorisinde ${n} ürün var. Silersen bu ürünler kategorisiz kalır. Devam edelim mi?`
      : `"${dispName(cat)}" kategorisini silmek istediğine emin misin?`
  ...
  async function save(form) {
    ...
    if (groupField) payload[groupField] = form.group || null
    if (hasOutlet) payload.outlet_id = form.outlet_id || null

    if (edit?.id) {
      await supabase.from('menu_categories').update(payload).eq('id', edit.id)
    } else {
      payload.sort_order = cats.length
      await supabase.from('menu_categories').insert(payload)
    }
    setShowForm(false); setEdit(null); load()
  }

  const colCount = 3 + (hasImage ? 1 : 0) + (groupField ? 1 : 0) + (statusField ? 1 : 0)
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 18, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: MUTED }}>{cats.length} kategori</p>
        </div>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          {hasOutlet && outlets.length > 0 && (
            <select value={outletFilter} onChange={e => setOutletFilter(e.target.value)} style={selectStyle}>
              <option value="">Tüm outletler (genel)</option>
              {outlets.map(o => <option key={o.id} value={o.id}>{o.name || dispName(o)}</option>)}
            </select>
          )}
          <button onClick={() => setReorder(r => !r)} style={btnGhost(reorder)}>
            <SortIcon /> {reorder ? 'Bitir' : 'Sırala'}
          </button>
      ...
      <div style={{ background: '#fff', border: `1px solid ${BORDER}`, borderRadius: 14, overflow: 'hidden' }}>
        {loading ? (
          ...
          <div style={{ padding: 56, textAlign: 'center', color: '#bbb' }}>
            ...
            <p style={{ fontSize: 14 }}>Henüz kategori yok. "Kategori Ekle" ile başla.</p>
          </div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#fafafa' }}>
                ...
                <th style={th}>Ad</th>
                {groupField && <th style={th}>Grup</th>}
                  ...
                  <tr key={cat.id} style={{ borderTop: `1px solid ${BORDER}` }}>
                    ...
                    {hasImage && (
                      <td style={td}>
                        {cat.image_url
                          ...
                          : <div style={{ width: 46, height: 46, borderRadius: 10, background: '#f4f4f2', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#ccc', fontSize: 18 }}>📁</div>}
                      </td>
                    )}
                    <td style={td}>
                      <p style={{ fontSize: 14, fontWeight: 600, color: '#111' }}>{dispName(cat)}</p>
                      {dispName(cat) !== (cat.name_en || '') && cat.name_en && <p style={{ fontSize: 11, color: '#bbb' }}>{cat.name_en}</p>}
                    </td>
                    {groupField && <td style={{ ...td, fontSize: 13, color: '#666' }}>{cat[groupField] || '—'}</td>}
                    <td style={td}>
                    ...
                    {statusField && (
                      <td style={{ ...td, textAlign: 'center' }}>
                        <Toggle on={!!cat[statusField]} onClick={() => toggleStatus(cat)} />
                      </td>
                    )}
      ...
      {showForm && (
        <CategoryForm cat={edit} dispName={dispName}
          hasImage={hasImage} statusField={statusField} groupField={groupField} groupOptions={groupOptions}
          hasOutlet={hasOutlet} outlets={outlets}
          onSave={save} onClose={() => { setShowForm(false); setEdit(null) }} />
      )}

      {/* schema ipucu */}
      {!loading && cats.length > 0 && (!hasImage || !statusField || !groupField) && (
        <p style={{ fontSize: 11.5, color: '#bbb', marginTop: 14, lineHeight: 1.6 }}>
          İpucu (reis): qrmenum'daki tüm sütunlar için <code>menu_categories</code> tablosuna şu kolonları ekleyebilirsin —
          {!hasImage && ' image_url (text),'}{!statusField && ' is_active (bool, default true),'}{!groupField && ' group (text).'}
          {' '}Kolon ekleyince ilgili sütun otomatik aktif olur.
        </p>
      )}
    </div>
  )
...
/* ── Kategori formu ── */
function CategoryForm({ cat, dispName, hasImage, statusField, groupField, groupOptions, hasOutlet, outlets, onSave, onClose }) {
  const [form, setForm] = useState({
    name_ka: cat?.name_ka || '', name_en: cat?.name_en || '',
    name_tr: cat?.name_tr || '', name_ru: cat?.name_ru || '',
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.4)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '60px 16px', zIndex: 50, overflowY: 'auto' }}>
      <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: 500, background: '#fff', borderRadius: 18, boxShadow: '0 20px 60px rgba(0,0,0,.3)' }}>
        ...
        <div style={{ padding: 22, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {(groupField || (hasOutlet && outlets.length > 0)) && (
            <div style={{ display: 'grid', gridTemplateColumns: groupField && hasOutlet ? '1fr 1fr' : '1fr', gap: 10 }}>
              {groupField && (
                <div>
          ...
          {statusField && (
            <label style={{ display: 'flex', alignItems: 'center', gap: 9, fontSize: 13, color: '#333', cursor: 'pointer', background: '#fafafa', borderRadius: 12, padding: 14 }}>
              <input type="checkbox" checked={form.active} onChange={e => set('active', e.target.checked)} style={{ width: 16, height: 16, accentColor: GREEN, cursor: 'pointer' }} />
              Aktif (menüde göster)
            </label>
          )}
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, padding: '16px 22px', borderTop: `1px solid ${BORDER}` }}>
          ...
          <button onClick={() => onSave(form)} disabled={!form.name_tr && !form.name_en}
            style={{ padding: '10px 22px', background: GREEN, color: '#fff', border: 'none', borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: 'pointer', boxShadow: '0 4px 12px rgba(29,158,117,.3)' }}>Kaydet</button>
        </div>
      </div>
    </div>
  )
...
function Toggle({ on, onClick }) {
  return (
    <button onClick={onClick} style={{ width: 38, height: 22, borderRadius: 11, border: 'none', cursor: 'pointer', background: on ? GREEN : '#d8d8d4', position: 'relative', transition: 'background .2s', padding: 0, verticalAlign: 'middle' }}>
...
const selectStyle = { padding: '10px 14px', border: `1px solid ${BORDER}`, borderRadius: 10, fontSize: 13, background: '#fff', cursor: 'pointer', minWidth: 180 }
...
function arrowBtn(disabled) {
  return { width: 26, height: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', border: `1px solid ${BORDER}`, borderRadius: 5, background: disabled ? '#f4f4f2' : '#fff', cursor: disabled ? 'default' : 'pointer', color: disabled ? '#ccc' : '#666', padding: 0 }
}
